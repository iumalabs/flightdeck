...
**Status note**: Implemented. Module 3 (Distributed tracing) was implemented and merged first,
satisfying this module's trace-correlation dependency (User Story 3) against real code, not just a
plan. This module's migration, `0004_structured_logs.sql`, landed after Module 3's
`0003_distributed_tracing.sql` as anticipated.

## Summary

Ship FlightDeck's third ingest surface: structured log lines, via the SAME envelope endpoint Modules
2-3 already extended (a new `"log"` item type, batching many records per submission per Sentry's
protocol — a materially different shape from one-error/one-transaction-per-item). Given log volume
is the highest of any data type FlightDeck ingests (illustrative: ~17M lines/day at a modest 10
req/sec × 20 lines/request), this module deliberately does NOT write one D1 row per log line —
...
**Primary Dependencies**: One new npm dependency, `aws4fetch` (AWS SigV4 request signing for
Workers) — used exclusively by the User Story 4 export-snapshot-copy path (research.md §8, corrected
during implementation), not the hot ingest path. Cloudflare Queues (a second queue, `LOG_INGEST`),
R2 (a shared static `LOGS` bucket plus dynamically-provisioned per-project export buckets), and a
new Durable Object class are platform bindings/API calls, not packages.

**Storage**: D1 (`log_batches`, `log_batches_fts` FTS5 virtual table, `log_batch_traces` — see
data-model.md); ONE shared, statically-bound `LOGS` R2 bucket for all projects' NDJSON log batches
(research.md §8's correction — mirrors Module 2's `SOURCE_MAPS` pattern: project isolation enforced
by D1-driven `project_id`-scoped queries, not R2 ACLs); one R2 bucket PER PROJECT, dynamically
created via the R2 API only when export access is first requested (User Story 4), holding a
point-in-time snapshot of that project's NDJSON log batches. A new Durable Object class, `LiveTail`
...
ingest itself extends `worker/modules/ingest/` for the same reason Module 3's transaction ingest did
— shared envelope endpoint, DSN auth, and (now per-category-keyed) rate limiter.
