...
implementation should not start before Module 2 (PR #7) merges, since this module directly extends
files that PR introduces.
...
- [ ] T001 [P] Create directory skeleton: `worker/modules/traces/`
- [ ] T002 Add `TRACE_INGEST` Queue producer binding, its consumer config
...
- [ ] T003 Create `worker/db/migrations/0003_distributed_tracing.sql` — additive
...
- [ ] T005 Add `isTransactionItem()` to `worker/modules/ingest/envelope.ts` alongside the existing
      `isEventItem()`, dispatching on the envelope item's `type` field (research.md §2) (depends on
      T001)
- [ ] T006 [P] Wire an empty `queue(batch, env, ctx)` handler export into `worker/index.ts`'s
      ...
      (depends on T001)
- [ ] T007 Verify `deno task build` and `deno check` still pass with the new bindings/tables/empty
...
- [ ] T008 [P] [US1] Write `tests/unit/transaction-dispatch.test.ts` (envelope item-type dispatch
...
- [ ] T009 [P] [US1] Write `tests/unit/waterfall-layout.test.ts` (a span's depth is correctly
...
- [ ] T010 [US1] **Spike**: verify Cloudflare Queues' local emulation under `wrangler dev`
...
- [ ] T011 [US1] Write `tests/contract/trace-ingest.spec.ts` (against real `wrangler dev`:
...
- [ ] T012 [US1] Implement transaction-item dispatch in `worker/modules/ingest/routes.ts`: after
...
- [ ] T013 [US1] Implement `worker/modules/ingest/trace-consumer.ts` — the `queue()` batch handler:
      ...
      `worker/index.ts`
- [ ] T014 [P] [US1] Implement `worker/modules/ingest/waterfall-layout.ts` — pure functions
...
- [ ] T015 [US1] Implement `GET /api/internal/traces/:id` in `worker/modules/traces/routes.ts` per
      ...
      T004)
- [ ] T016 [US1] Wire the real `tracesRoutes` into `worker/index.ts` under `/api/internal/traces`,
      replacing T006's empty stub (depends on T015, T006)
- [ ] T017 [P] [US1] Create `app/shell/TraceDetailScreen.tsx` — visual waterfall using T014's
...
- [ ] T018 [US1] Add `selectedTraceId` state to `app/shell/AppShell.tsx` (mirrors research.md §11's
      `selectedIssueId` pattern); wire a `"trace-detail"` screen case routing to
      `TraceDetailScreen` (depends on T017)
- [ ] T019 [US1] Run T008-T011's tests, confirm all pass (depends on T012-T018)
...
- [ ] T020 [P] [US2] Write `tests/unit/percentiles.test.ts` (the `ORDER BY`/`OFFSET` query-builder
...
- [ ] T021 [US2] Implement `worker/modules/ingest/percentiles.ts` — pure functions building the
      `ORDER BY duration_ms ASC LIMIT 1 OFFSET ...` query per research.md §7's exact pattern
      (depends on T020)
- [ ] T022 [US2] Implement `GET /api/internal/traces` in `worker/modules/traces/routes.ts` —
...
- [ ] T023 [US2] Replace `app/shell/TracesScreen.tsx`'s static empty state with the real grouped
      ...
      T022, T018)
- [ ] T024 [US2] Run T020's test, confirm it passes (depends on T021)
...
- [ ] T025 [P] [US3] Write a unit test (`tests/unit/trace-context-extraction.test.ts`) for a new
...
- [ ] T026 [US3] Implement `extractTraceContext()` (in `worker/modules/ingest/routes.ts` or a small
      ...
      T025, T004)
- [ ] T027 [US3] Add a `traceId` field to `GET /api/internal/issues/:id`'s response in
...
- [ ] T028 [US3] Implement the real `linkedErrors` lookup (`SELECT ... FROM events WHERE trace_id =
      ?` joined to `issues`) in `GET /api/internal/traces/:id`, replacing T015's `[]` stub (depends
      on T026, T015)
- [ ] T029 [P] [US3] Add a "View trace" link to `app/shell/IssueDetailScreen.tsx`, shown when
      `traceId` is present (absent — not an error state — otherwise, per spec.md FR-009) (depends
      on T027)
- [ ] T030 [P] [US3] Add a linked-error(s) section to `app/shell/TraceDetailScreen.tsx`, shown when
...
- [ ] T032 Extend `worker/modules/ingest/retention.ts` to also prune `transactions` rows (full row,
      including `spans_json`) past their own 30-day window, on the same `scheduled()` cron trigger
      (research.md §8)
- [ ] T033 [P] Write a unit test for the transactions-retention query logic (prunes old
      transactions, leaves recent ones) in `tests/unit/retention.test.ts` (extending the existing
      file) or a new sibling
- [ ] T034 [P] Write `tests/e2e/traces-list-and-waterfall.spec.ts` — pre-authenticated context plus
...
- [ ] T035 [P] Run `deno fmt` and `deno lint` across `worker/`, `app/`, `tests/`; fix violations
- [ ] T036 [P] Run `deno check` (typecheck) across every new/changed `.ts`/`.tsx` file
- [ ] T037 Run the full `quickstart.md` validation end-to-end (all three user stories) and record
      results
- [ ] T038 Update `README.md`'s Status section to reference `specs/003-distributed-tracing`;
